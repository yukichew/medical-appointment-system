...
if (isset($_POST['submit'])) {
    ...
    if (
        isset($doctor_email, $currentPassword, $doctor_password, $doctor_name, $doctor_contact, $doctor_qualification, $doctor_specialist, $doctor_status, $doctor_dob)
        && !empty($doctor_email) && !empty($doctor_name) && !empty($doctor_qualification) && !empty($doctor_specialist) && !empty($doctor_status) && !empty($doctor_dob) && !empty($doctor_password) && !empty($currentPassword)
    ) {
        ...
        $sql = "SELECT doctor_id FROM doctor WHERE doctor_id = '$doctorID' and doctor_password = '$currentPassword'";
        ...
        if (mysqli_affected_rows($conn) == 0) {
            ...
            $sql = "UPDATE doctor SET doctor_email = '$doctor_email', doctor_email = '$doctor_email', doctor_name = '$doctor_name' , doctor_contact = '$doctor_contact' , doctor_qualification = '$doctor_qualification' 
            , doctor_specialist = '$doctor_specialist' , doctor_status = '$doctor_status'  , doctor_password = '$doctor_password' WHERE doctor_id = " . $doctorID;
            ...
            if (mysqli_query($conn, $sql)) {
                if (!mysqli_query($conn, $sql)) {
                    ...
                    echo '<script>alert("Doctor details have been updated successfully."); window.location.href = "doctorProfile.php"; </script>';
                }
            }
        }
    }
...
$selectOrder_sql = 'SELECT * from doctor WHERE doctor_id = ' . $doctorID;
                    ...
                    <div class="card">
                        <div class="card-body pt-3" id="editDoctor">
                            <form method="POST">
                                ...
                                <div class="row mb-3">
                                    <label for="doctor_password" class="col-md-4 col-lg-3 col-form-label">New Password</label>
                                    <div class="col-md-8 col-lg-9">
                                        <input name="doctor_password" type="password" class="form-control" id="doctor_password" required>
                                    </div>
                                </div>
